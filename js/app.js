(() => {
    ...
    const slider = element => {
        ...
        let priceGap = 1e3;
        rangeInput[0].parentNode.appendChild(minPriceSpan);
        rangeInput[1].parentNode.appendChild(maxPriceSpan);
        const initialState = {
            minPriceSpan: 100,
            maxPriceSpan: 1e4
        };
        const updateSpans = () => {
            const minPrice = priceInput[0].value ? parseInt(priceInput[0].value) : initialState.minPriceSpan;
            const maxPrice = priceInput[1].value ? parseInt(priceInput[1].value) : initialState.maxPriceSpan;
            minPriceSpan.textContent = minPrice;
            maxPriceSpan.textContent = maxPrice;
            minPriceSpan.style.left = rangeInput[0].value / rangeInput[0].max * 100 + "%";
            maxPriceSpan.style.left = rangeInput[1].value / rangeInput[1].max * 100 + "%";
        };
    ...
    document.addEventListener("DOMContentLoaded", (function() {
        if (window.location.pathname.includes("single-room.html")) if (typeof Splide !== "undefined") {
        ...
        } else console.error("Splide is not loaded");
    }));
    $((function() {
        function setupDatepicker(selector, container) {
            $(selector).datepicker({
                ...
                beforeShow: function(input, inst) {
                    ...
                    $(".booking-btn").css("margin-top", "235px");
                },
                onClose: function(input, inst) {
                    inst.dpDiv.removeClass("active");
                    $(".booking-btn").css("margin-top", "0");
                }
            });
        }
        setupDatepicker("#checkin", "#checkin-container");
        setupDatepicker("#checkout", "#checkout-container");
    }));
    window["FLS"] = true;
    isWebp();
})();